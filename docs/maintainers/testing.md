...
Coverage is a **signal**, not a goal: prioritize tests for **pure helpers** (URL/query builders, body maps, [`internal/cli/trayref`](../../internal/cli/trayref), format flags without cobra), decoding, and HTTP adapters (`httptest`). Integration-style tests only where they add confidence without flaking.
